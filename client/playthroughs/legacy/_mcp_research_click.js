...
function printInteractables(analysis) {
    ...
    for (const c of interactables) {
        ...
        log(`    "${c.text ? c.text.trim() : '(no text)'}"${disabled} path=${c.path}`);
    }
}
...
async function main() {
    ...
    // Try to find "Orbitales Werft" in the UI
    log("=== Finding Orbitales Werft-Design ===");
    let found = await client.tool("runtime_ux_find", { description: "Orbitales Werft-Design" });
    log(`Find "Orbitales Werft-Design": ${JSON.stringify(found)}`);

    if (!found.found) {
        // Try shorter match
        found = await client.tool("runtime_ux_find", { description: "Orbitales Werft" });
        log(`Find "Orbitales Werft": ${JSON.stringify(found)}`);
    }

    if (!found.found) {
        // Try "Werft-Design"
        found = await client.tool("runtime_ux_find", { description: "Werft-Design" });
        log(`Find "Werft-Design": ${JSON.stringify(found)}`);
    }

    if (found.found) {
        log(`Found at center: ${JSON.stringify(found.center)}`);
        log("=== Clicking to research ===");
        const clicked = await client.tool("runtime_ux_click", { description: "Orbitales Werft" });
        log(`Clicked: ${JSON.stringify(clicked)}`);
        await sleep(1000);

        // Verify it was researched
        ...
        fs.writeFileSync("_mcp_research_click.json", JSON.stringify({ found, clicked, analysis }, null, 2));
    } else {
        log("Could not find Orbitales Werft-Design button");
        // List all interactable controls
        const analysis = await client.tool("runtime_ux_analyze", { include_visual: true });
        printInteractables(analysis);
        fs.writeFileSync("_mcp_research_click.json", JSON.stringify({ analysis }, null, 2));
    }
