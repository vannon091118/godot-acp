...
async function main() {
    ...
    // First, close the PLANETEN panel
    log("=== Closing PLANETEN panel ===");
    const closeClicked = await client.tool("runtime_ux_click", { description: "SCHLIESSEN" });
    log(`SCHLIESSEN clicked: ${closeClicked.clicked}`);
    await sleep(500);

    // Click FORSCHUNG (Research)
    log("=== Clicking FORSCHUNG (Research) ===");
    const clicked = await client.tool("runtime_ux_click", { description: "FORSCHUNG" });
    log(`FORSCHUNG clicked: ${clicked.clicked}`);
    await sleep(2000);

    // Analyze the research menu
    ...
    fs.writeFileSync("_mcp_research_view.json", JSON.stringify(analysis, null, 2));
    log("=== Done ===");
}
